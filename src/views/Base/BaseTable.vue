    ...
    >
      ...
      <template v-slot:item.actions="{ item , index}">
        ...
        <div v-else>
          <base-btn
              button-is-icon
              :menu-items="getMenuFinalAction(finalActions, item)"
          />
        </div>
      </template>
      <template v-slot:item.creationTime="{ item }">
        <div> {{ getPersianTime(item.creationTime) }}</div>
      </template>
      <template
...
import BaseSquareButton from "@/view/widget/Base/BaseSqaureButton.vue";
...
export default {
  ...
  created() {
    ...
    if (this.defaultActions === true) {
      this.finalActions.push({
            title: 'ویرایش',
            icon: 'mdi-pen',
            click: item => {
              this.$emit('edit', {item, index: this.items.indexOf(item)})
            },
          },
          {
            title: "حذف",
  ...
  methods: {
    ...
    getMenuFinalAction(menuItems, rowItem) {
      return menuItems.map(f => {
        return {
          click: () => {
            f.click(rowItem)
          },
